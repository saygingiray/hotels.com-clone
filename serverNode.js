...
app.get("/similarHotels", async (req, res) => {
    ...
    let temp_str = "address.location.coordinates.0"; queryX[temp_str] = { $gte: Number(req.query.lng0), $lte: Number(req.query.lng1) }
    let temp_str2 = "address.location.coordinates.1"; queryX[temp_str2] = { $gte: Number(req.query.lat0), $lte: Number(req.query.lat1) }

    optionsX.projection = {
        name: 1, 'address.location.coordinates': 1, "address.market": 1, "property_type": 1, "room_type": 1,
        "bed_type": 1, "accommodates": 1, "number_of_reviews": 1, "review_scores.review_scores_rating": 1, "price": 1, "images.picture_url" : 1};
    optionsX.limit = 20

    try {
        await client.connect();
        const dataDBX4 = await collections.find(queryX, optionsX).toArray();
        res.json(dataDBX4);

        console.log(dataDBX4)
    }
    catch (err) {
        console.log(err);
    }
